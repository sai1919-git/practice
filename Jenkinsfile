pipeline {
    ...
    stages{
        stage('checkout'){
            steps{
                git branch: 'main',url: 'https://github.com/sai1919-git/practice.git'
            }
        }
        ...
        stage('push to docker'){
            steps{
                withCredentials([usernamePassword(credentialsId: 'ee113df6-4efb-439e-a879-57dcd713dd00',usernamepipeline {
    agent any
    environment {
        IMAGE_NAME = "sai604/mydocker"
    }
...
Variable: 'DOCKER_USER',passwordVariable: 'DOCKER_PASS')]){
                    sh """
                   echo "$DOCKER_PASS" | docker login -u "$DOCKER_USER" --password-stdin
                    docker push $IMAGE_NAME:latest
                    docker logout
